...
class CPennTaggedSentenceReader(object):
    ...
    def readSentence(self, bIgnoreNoneTag):
        ...
        for nIndex in xrange(len(lLine)):
            ...
            if len(tTagged) == 1:
                tTagged = (tTagged[0], "-NONE-")
            if (bIgnoreNoneTag == False) or (
                    tTagged[0]):  # if we take -NONE- tag, or if we find that the tag is not -NONE-
                lNewLine.append(tTagged)
        return lNewLine
...
def evaluate(sCandidate, sReference):
    #
...
#   print "%.5f,%.5f,%.5f" % (word_precision, word_recall, word_fmeasure)
